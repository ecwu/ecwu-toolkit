...
export default function TextCleanupPage() {
  ...
  const cleanText = () => {
    ...
    chars.forEach(char => {
      ...
      if (isProblematic !== isCurrentSegmentProblematic || 
          problemType !== currentProblemType ||
          (isProblematic && JSON.stringify(problemData) !== JSON.stringify(currentProblemData))) {
        flushSegment();
      ...
      <div className="flex flex-1 flex-col gap-4 p-4">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Remove hidden Unicode characters, normalize special characters, and clean up text from LLM outputs.
            Perfect for cleaning up copied text with invisible formatting characters.
          </p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
                ...
                {result && (
                  <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded-md border">
                    ...
                    <div className="text-sm text-blue-600 dark:text-blue-300">
                      Found {result.highlightedText.filter(segment => segment.isProblematic).reduce((sum, segment) => sum + segment.text.length, 0)} problem characters
                    </div>
                  </div>
                )}
              </div>
              ...
              {result && result.highlightedText && result.highlightedText.length > 0 && (
                <div className="space-y-2 lg:col-span-1">
                  <label className="text-sm font-medium">Original Text with Highlights</label>
                  <div className="p-3 border rounded-md bg-muted/50 min-h-32 font-mono text-sm whitespace-pre-wrap break-words">
                    {result.highlightedText.map((segment, index) => {
                      ...
                      const displayText = segment.text === ' ' ? '␣' : segment.text;
                              ...
                              <div className="flex items-start gap-2">
                                ...
                                <div className="space-y-2">
                                  <h4 className="font-medium text-sm">
                                    {isReplacement ? 'Character Replacement' : 'Character Removal'}
                                  </h4>
                                  
                                  <div className="space-y-1 text-sm">
                                    <div>
                                      <span className="text-muted-foreground">Problem:</span> {segment.description}
                                    </div>
                                    
                                    <div>
                                      <span className="text-muted-foreground">Unicode:</span> {segment.unicodeCode}
                                    </div>
                                    
                                    <div>
                                      <span className="text-muted-foreground">Original character:</span>{' '}
                                      <code className="bg-muted px-1 rounded text-xs">
                                        {segment.original === ' ' ? '␣ (space)' : `"${segment.original}"`}
                                      </code>
                                    </div>
                                    
                                    {isReplacement && (
                                      <div>
                                        <span className="text-muted-foreground">Will be replaced with:</span>{' '}
                                        <code className="bg-muted px-1 rounded text-xs">
                                          {segment.replacement === ' ' ? '␣ (space)' : `"${segment.replacement}"`}
                                        </code>
                                      </div>
                                    )}
                                    ...
                                    {!isReplacement && (
                                      <div className="text-red-600 dark:text-red-400">
                                        <span className="text-muted-foreground">Action:</span> This character will be removed
                                      </div>
                                    )}
                                  </div>
                                </div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    ...
                    <div className="mt-1">Click highlighted characters for details</div>
                  </div>
                </div>
              )}
